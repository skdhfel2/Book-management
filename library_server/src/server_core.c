...
unsigned WINAPI HandleClient(void *arg)
{
    ...
    while ((strLen = recv(clientSock, msg, BUF_SIZE - 1, 0)) > 0)
    {
        ...
        else if (strcmp(sub_cmd, CMD_ADD_USER) == 0)
        {
            ...
            // 새 사용자 추가
            strncpy(users[user_count][0], id, MAX_USER_LEN - 1);
            users[user_count][0][MAX_USER_LEN - 1] = '\0';

            strncpy(users[user_count][1], pw, MAX_USER_LEN - 1);
            ...
            if (save_users_from_memory(users, user_count + 1))
            {
                ...
                send(clientSock, "SL//1 0\n", strlen("SL//1 0\n"), 0);
                char msg[BUF_SIZE];
                snprintf(msg, sizeof(msg), "MSG//사용자 '%s'가 추가되었습니다.\n", id);
        ...
        else if (strcmp(sub_cmd, CMD_DELETE_USER) == 0)
        {
            ...
            if (found && save_users_from_memory(users, user_count - 1))
            {
                ...
                send(clientSock, "SL//1 0\n", strlen("SL//1 0\n"), 0);
                char msg[BUF_SIZE];
                snprintf(msg, sizeof(msg), "MSG//사용자 '%s'가 삭제되었습니다.\n", id);
                send(clientSock, msg, strlen(msg), 0);
            }
            else
            {
        ...
        else if (strcmp(sub_cmd, CMD_UPDATE_USER) == 0)
        {
            ...
            if (found && save_users_from_memory(users, user_count))
            {
                ...
                send(clientSock, "SL//1 0\n", strlen("SL//1 0\n"), 0);
                char msg[BUF_SIZE];
                snprintf(msg, sizeof(msg), "MSG//사용자 '%s'의 비밀번호가 수정되었습니다.\n", id);
                send(clientSock, msg, strlen(msg), 0);
            }
            else
            {
        ...
        else if (strcmp(sub_cmd, CMD_SEARCH_BOOK) == 0)
        {
            ...
            if (!found)
            {
                ...
                send(clientSock, response, strlen(response), 0);
                // 사용자에게 못찾았다고 메시지 보냄
                send(clientSock, "MSG//검색 결과가 없습니다.\n", strlen("MSG//검색 결과가 없습니다.\n"), 0);
            }
            else
            {
        ...
        else if (strcmp(sub_cmd, CMD_ADD_BOOK) == 0)
        {
            ...
            if (save_books_from_memory(books, book_count + 1))
            {
                ...
                // 사용자에게 친절한 메시지 전송
                char msg[BUF_SIZE];
                snprintf(msg, sizeof(msg), "MSG//도서 '%s' 추가가 완료되었습니다.\n", title);
                send(clientSock, msg, strlen(msg), 0);
            }
            else
            {
        ...
        else if (strcmp(sub_cmd, CMD_UPDATE_BOOK) == 0)
        {
            ...
            if (found && save_books_from_memory(books, book_count))
            {
                ...
                send(clientSock, "SL//1 0\n", strlen("SL//1 0\n"), 0);
                char msg[BUF_SIZE];
                snprintf(msg, sizeof(msg), "MSG//도서 번호 %d가 수정되었습니다.\n", id);
                send(clientSock, msg, strlen(msg), 0);
            }
            else
            {
        ...
        else if (strcmp(sub_cmd, CMD_DELETE_BOOK) == 0)
        {
            ...
            if (found && save_books_from_memory(books, book_count - 1))
            {
                ...
                // 도서삭제 메시지
                char msg[BUF_SIZE];
                snprintf(msg, sizeof(msg), "MSG//도서 번호 %d가 삭제되었습니다.\n", id);
                send(clientSock, msg, strlen(msg), 0);
            }
            else
            {
